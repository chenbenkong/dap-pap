...
- **高亮开关（补光式单件高亮）**：装配台 / 试车台各有一个 `高亮部件` 开关，默认关。打开后**点击哪个、只高亮哪个**（不累计、不残留）。高亮是一束跟随该部件的聚光——原材质 / 贴图 / 颜色 100% 保留，只是被额外照得更亮、更有"光感"，绝不用一层橙色覆盖挡住外观。部件随爆炸 / 自动旋转移动时，灯位每帧按相机方向重算，始终从观察者这一侧打光。
...
| **第三人称** `CHASE` | 60–85 m | 挂在弹后上方，弹体占画面约 60%，末段视线自动压向目标舰 |
| **电影机位** `CINE` | 150–200 m | 侧后方缓慢环绕，看清扫全貌与弹目相对位置 |
