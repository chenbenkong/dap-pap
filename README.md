...
| **第三人称** `CHASE` | 76–134 m | 侧后方约 33° 中远景跟拍（视场 48°~50°），海面/弹道/发射台都入画，导弹约占画面三分之一；起飞仰拍 → 滑翔全景 → 末段弹目同框 |
| **电影机位** `CINE` | 84–152 m | **默认机位**：中远景分段镜头——起飞低角度仰拍整弹立架、助推侧影、滑翔大远景、末段弹目同框，导弹始终是主体但四周景物清楚可见 |
...
全程任务弹道 MissionSim
  ...
  ✓ 阶段时序单调递增  助推→7.2s  滑翔→46.5s  命中→72.6s
  ✓ 末段落点偏差 < 2% 射程  落点 x=27.20km  目标 27.2km  偏差 0m
  ✓ 轴向过载落在弹体结构可承受区间  峰值 16.5 G（助推段 16.5 G）
