...
### 第 2 章 · 动力原理
- 点火试车：推力曲线实时绘制，尾焰（含马赫盘/激波菱形）+ 火花粒子 + 动态点光源
...
- 🎬 章节间相机飞行动画与转场提示
- 🐛 调试利器：URL 加 `#view=x,y,z|tx,ty,tz` 可直取任意相机机位（方便截图与分享）
...
| 后期 | EffectComposer + UnrealBloom + OutputPass + FXAA | 官方后期链，ACES 色调映射 |
...
- **PBR 程序化蒙皮**：Canvas 生成 1024×2048 反照率图（舱段缝 + 双排铆钉 + 检修盖板 + NO STEP / LIFT HERE 喷涂 + 磨损流痕）、粗糙度图、以及由高度图 Sobel 算法导出的法线图；UV 沿弹体全局坐标映射，8 个舱段爆炸分离后漆面依然连续
- **真实气动外形**：切线卵形（tangent ogive）头部曲线、船尾收锥、拉瓦尔喷管（Catmull-Rom 样条平滑轮廓）、带翼型剖面的后掠弹翼 + 翼尖滚转舵（rolleron）
