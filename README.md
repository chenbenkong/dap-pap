...
- **高亮开关（部件自发光）**：装配台 / 试车台各有一个 `高亮部件` 开关，默认关。打开后**点击哪个、只高亮哪个**（不累计、不残留）。高亮是让该部件**自发光**——在材质本色基础上透出一层冷白光，呈现"透亮"的观感，强度克制（0.6），能一眼分辨出是当前选中件即可，不会盖住蒙皮、铆钉与喷印细节。
...
| 机位 | 距离 | 适合看什么 |
|---|---|---|
| **第一人称** `FPV` | 15 m | 骑在弹背上，视野随速度张开（Ma 3 时 FOV 推到 92°），速度感最强 |
| **第三人称** `CHASE` | 56–72 m | 侧后方约 33° 跟拍，弹体以**侧影**入画（占画面 39%~88%），末段视线自动压向目标舰 |
| **电影机位** `CINE` | 79–99 m | 侧后方 36°~84° 缓慢环绕，看清扫全貌与弹目相对位置（占画面 29%~68%） |
| **全局** `GLOBAL` | 战区尺度 | 俯瞰完整弹道，菱形光标标出导弹位置 |
| **自由** `FREE` | 交还鼠标 | 自己拖着看，适合观察爆炸与目标舰 |

跟拍机位由本模块直接驱动相机（绕开 OrbitControls，否则 `controls.update()` 会用 target 反推球坐标、把手动设置的机位改写回去）。
