...
| 机位 | 距离 | 适合看什么 |
|---|---|---|
| **第一人称** `FPV` | 贴弹 | **纯导引头视角**：机位贴在弹头前端看向正前方（来袭方向），弹身完全在身后，速度感 FOV 随马赫张开 |
| **第三人称** `CHASE` | 57–69 m | 侧后方约 33° 跟拍，弹体以**侧影**入画（占画面 40%~85%），末段视线自动压向目标舰 |
| **电影机位** `CINE` | 72–88 m | **默认机位**：侧后方 36°~84° 缓慢环绕，看清扫全貌与弹目相对位置（占画面 29%~68%） |
| **全局** `GLOBAL` | 战区尺度 | 按真实弹道包围盒自动取景、缓慢环绕，俯瞰完整弹道，菱形光标标出导弹位置 |
